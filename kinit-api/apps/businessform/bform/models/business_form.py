...
class BusinessForm(Base):
    ...
    create_datetime = mapped_column(DateTime, nullable=True, server_default=func.now(), comment='创建时间')
    update_datetime = mapped_column(DateTime, nullable=True, server_default=func.now(), onupdate=func.now(), comment='更新时间')
    is_delete = mapped_column(Integer, nullable=True, default=0, comment='是否删除：0-否, 1-是')