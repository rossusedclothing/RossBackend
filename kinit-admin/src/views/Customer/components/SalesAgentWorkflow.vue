  ...
  <div class="p-4">
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center space-x-2">
        ...
        <el-button type="primary" @click="onOpenCreate">创建工作流程</el-button>
      </div>
    ...
    <el-dialog
      ...
      width="700px"
      :title="'工作流程 - ' + (viewDialog.data?.name || '')"
    >
      <div class="space-y-3">
        ...
        <el-input type="textarea" :rows="8" :model-value="viewDialog.data?.config" disabled />
      </div>
      <template #footer>
    ...
    <el-dialog
      ...
      width="720px"
      :title="editDialog.isEdit ? '编辑工作流程' : '创建工作流程'"
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px" class="space-y-2">
        ...
        /></el-form-item>
        <el-form-item label="配置信息(JSON或文本)" prop="config">
          <el-input type="textarea" :rows="10" v-model="form.config" />
        </el-form-item>
      </el-form>
...
const viewDialog = reactive({ visible: false, data: null as WorkflowItem | null })
...
const onView = (row: WorkflowItem) => {
  if (row?.id) {
    getSalesAgentWorkflowInfoApi(row.id)
      .then((res: any) => {
  ...
  ;(formRef.value as any)?.validate?.(async (valid: boolean) => {
    ...
    try {
      if (editDialog.isEdit && editDialog.data_id) {
        await updateSalesAgentWorkflowApi(editDialog.data_id, { ...form })
        ElMessage.success('更新成功')
      } else {
        await createSalesAgentWorkflowApi({ ...form })
...
const onDel = (row: WorkflowItem) => {
  ElMessageBox.confirm('确认删除该工作流程吗？', '删除确认', { type: 'warning' })
    .then(async () => {
      try {
        await deleteSalesAgentWorkflowApi([row.id])
