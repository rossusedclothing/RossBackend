  ...
  <div class="public-form-container">
    ...
    <div class="form-header">
      <div class="header-content">
        <h1 class="form-title">{{ isEditMode ? '编辑客户信息' : '客户信息登记表' }}</h1>
        <p class="form-description">{{ isEditMode ? '修改客户信息' : '欢迎填写客户信息，我们将尽快与您联系' }}</p>
      </div>
    </div>
    ...
    >
      <div class="success-content">
        ...
        <p class="success-message">
          {{ isEditMode ? '信息更新成功！' : '感谢您的提交！我们会在24小时内与您联系。' }}
        </p>
      </div>
      <template #footer>
        <el-button type="primary" @click="handleSuccessConfirm" size="large">
          {{ isEditMode ? '返回列表' : '继续填写新表单' }}
        </el-button>
      </template>
    </el-dialog>
  </div>
...
<script setup lang="ts">
  ...
  import { createBusinessForm, getBusinessForm, updateBusinessForm } from '@/api/businessform/list/list'
  ...
  const isEditMode = ref(false) // 新增：编辑模式标志
  const recordId = ref<number | null>(null) // 新增：记录ID
  const originalData = ref({}) // 新增：保存原始数据用于恢复
  ...
  const loadRecordData = async (id: number) => {
    try {
      loading.value = true
      const response = await getBusinessForm(id)
  ...
  // 页面加载时设置标题和处理参数
  onMounted(async () => {
    const { id, bt } = getUrlParams()

    // 设置页面标题
    document.title = id ? '编辑客户信息' : '客户信息登记表'

    // 处理编辑模式
    if (id) {
      const recordIdNum = Number(id)
      if (!isNaN(recordIdNum) && recordIdNum > 0) {
        isEditMode.value = true
        recordId.value = recordIdNum
        await loadRecordData(recordIdNum)
      } else {
    ...
    // 处理bt参数
    if (bt) {
      if (validateBtParam(bt)) {
        formData.bt = bt
        console.log('有效的bt参数:', bt)
      } else {
        ElMessage.error('分享链接参数错误，请联系分享人获取正确的链接')
        console.error('无效的bt参数:', bt)
      }
    } else if (currentUser.value?.id) {
      // 如果没有bt参数但有当前用户，使用当前用户ID
      formData.bt = currentUser.value.id.toString()
    }
  })
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      if (isEditMode.value && recordId.value) {
        // 编辑模式：调用更新接口
        response = await updateBusinessForm(recordId.value, submitData)
      } else {
        // 新增模式：调用创建接口
        response = await createBusinessForm(submitData)
      }

      if (response.code === 200) {
        successDialogVisible.value = true
      } else {
        ElMessage.error(response.message || (isEditMode.value ? '更新失败，请重试' : '提交失败，请重试'))
      }
    } catch (error: any) {
  ...
  // 重置表单
  const resetForm = () => {
    if (isEditMode.value && originalData.value) {
      // 编辑模式：恢复原始数据
      ElMessageBox.confirm('确定要恢复原始数据吗？', '提示', {
  ...
  const handleSuccessConfirm = () => {
    ...
    if (isEditMode.value) {
      // 编辑模式：返回列表或留在当前页面
      router.push('/businessform/list') // 假设有列表页面
    } else {
      // 新增模式：重置表单
      resetForm()
